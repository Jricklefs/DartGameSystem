...
class DetectionClient:
    ...
    def detect(self, camera_id: str, frame: np.ndarray) -> DetectionResponse:
        ...
        """
        try:
            # Hub expects: { boardId, images: [{cameraId, image}] }
            payload = {
    ...
    def notify_dart_detected(self, dart_index: int):
        ...
        try:
            self.session.post(
                self._url("/api/games/events/dart"),
                json={"boardId": self.board_id, "dartIndex": dart_index},
                timeout=2
            )
    ...
    def notify_board_clear(self):
        ...
        try:
            self.session.post(
                self._url("/api/games/events/clear"),
