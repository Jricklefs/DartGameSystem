...
function drawPolygonOverlay(ctx, cal, poly, cx, cy) {
    ...
    drawRingPolygon(dOuters, 'rgba(255,255,0,0.8)', 2);
    drawRingPolygon(dInners, 'rgba(255,200,0,0.6)', 1.5);
    drawRingPolygon(tOuters, 'rgba(255,255,0,0.8)', 2);
    drawRingPolygon(tInners, 'rgba(255,200,0,0.6)', 1.5);
    ...
    if (dOuters[0] && dOuters[1]) {
        ctx.lineTo(dOuters[0][0], dOuters[0][1]);
        ctx.lineTo(dOuters[1][0], dOuters[1][1]);
    }
    ctx.closePath();
    ...
    // Draw segment boundary lines from center to outer double + 30px extension
    for (let i = 0; i < 20; i++) {
        const outerPt = dOuters[i];
        if (!outerPt) continue;
        
        // Extend line 30px past the outer double point
        const dx = outerPt[0] - cx;
        const dy = outerPt[1] - cy;
        const len = Math.hypot(dx, dy);
        const extendPx = 30;
        const endX = outerPt[0] + (dx / len) * extendPx;
        const endY = outerPt[1] + (dy / len) * extendPx;
        
        // Line from center to extended point
        ctx.beginPath();
        ctx.moveTo(cx, cy);
        ctx.lineTo(endX, endY);
        
        // Highlight segment 20 boundaries (boundary 0 and 1 border segment 20)
        if (i === 0 || i === 1) {
        ...
        drawDot(dOuters[i], 'rgba(255,255,0,0.8)', 3);
        if (dInners && dInners[i]) drawDot(dInners[i], 'rgba(255,200,0,0.6)', 2);
        if (tOuters && tOuters[i]) drawDot(tOuters[i], 'rgba(255,255,0,0.8)', 3);
        if (tInners && tInners[i]) drawDot(tInners[i], 'rgba(255,200,0,0.6)', 2);
    }
    ...
    for (let i = 0; i < 20; i++) {
        ...
        // Midpoint of outer boundary points, pushed outward
        ...
        // Label ~50px beyond the outer ring
        const labelX = midX + (dx / len) * 50;
        const labelY = midY + (dy / len) * 50;
...
// ============== Camera Settings ==============
let selectedSettingsCam = 0;
let camPreviewInterval = null;

function selectSettingsCam(cam) {
    selectedSettingsCam = cam;
    document.querySelectorAll('.cam-settings-btn').forEach(b => {
        b.classList.toggle('active', b.dataset.cam == String(cam));
    });
...
function startCamPreview() {
    if (camPreviewInterval) clearInterval(camPreviewInterval);
    const img = document.getElementById('cam-preview-img');
    const camIdx = selectedSettingsCam === 'all' ? 0 : selectedSettingsCam;
    
    function updatePreview() {
        // Use preview endpoint for direct JPEG
        const url = DART_SENSOR_URL + '/v1/camera/preview?camera=' + camIdx + '&t=' + Date.now();
        img.src = url;
    }
    updatePreview();
    camPreviewInterval = setInterval(updatePreview, 1000); // 1fps preview
}

function stopCamPreview() {
    if (camPreviewInterval) { clearInterval(camPreviewInterval); camPreviewInterval = null; }
}

function updateExposureLabel(val) {
...
async function applyCameraSetting() {
    ...
    const camParam = selectedSettingsCam === 'all' ? '' : '?camera=' + selectedSettingsCam;
    const status = document.getElementById('cam-settings-status');
    status.textContent = 'Applying...';
    status.style.color = 'var(--gold)';
    
    try {
        const resp = await fetch(DART_SENSOR_URL + '/v1/camera/set' + camParam, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.results && data.results.length > 0) {
            ...
            if (r.current) {
                document.getElementById('cam-brightness').textContent = 
                    r.current.brightness !== undefined ? r.current.brightness.toFixed(1) : '--';
            }
        }
    } catch (e) {
        status.textContent = 'DartSensor not reachable';
...
async function refreshCameraSettings() {
    try {
        const resp = await fetch(DART_SENSOR_URL + '/v1/camera/settings');
        const data = await resp.json();
        const status = document.getElementById('cam-settings-status');
        
        if (data.cameras) {
            const camIdx = selectedSettingsCam === 'all' ? 0 : selectedSettingsCam;
            const cam = data.cameras.find(c => c.camera === camIdx) || data.cameras[0];
            if (cam) {
                const exp = cam.exposure !== undefined ? cam.exposure : -7;
        ...
        document.getElementById('cam-settings-status').textContent = 'DartSensor not reachable';
        document.getElementById('cam-settings-status').style.color = '#f44336';
    }
}

async function autoTuneCameras() {
    const btn = document.getElementById('auto-tune-btn');
    const status = document.getElementById('cam-settings-status');
    const resultsDiv = document.getElementById('auto-tune-results');
    const contentDiv = document.getElementById('auto-tune-content');
    
    btn.disabled = true;
    btn.textContent = 'Tuning...';
    status.textContent = 'Auto-tuning cameras (sweeping exposure values)...';
    status.style.color = 'var(--gold)';
    
    const camParam = selectedSettingsCam === 'all' ? '' : '?camera=' + selectedSettingsCam;
    
    try {
        const resp = await fetch(DART_SENSOR_URL + '/v1/camera/auto-tune' + camParam, {
            method: 'POST'
        });
        const data = await resp.json();
        ...
        if (data.results) {
            ...
            data.results.forEach(r => {
                if (r.optimal) {
                    ...
                    html += '<strong style="color: var(--paper);">Camera ' + r.camera + '</strong>: ';
                    html += 'Exposure = <span style="color: var(--gold);">' + r.optimal.exposure + '</span>, ';
                    html += 'Brightness = ' + r.optimal.mean_brightness + ', ';
                    html += 'Contrast = ' + r.optimal.std_dev;
                    html += '</div>';
            ...
            status.textContent = 'Auto-tune complete!';
            status.style.color = '#4caf50';
        }
    } catch (e) {
        status.textContent = 'Auto-tune failed: ' + e.message;
        status.style.color = '#f44336';
    }
...
function resetCameraDefaults() {
    // Re-enable auto exposure on all cameras
    document.getElementById('auto-exposure-toggle').checked = true;
    document.getElementById('auto-wb-toggle').checked = true;
    document.getElementById('exposure-val').textContent = 'Auto';
    document.getElementById('gain-slider').value = 0;
    document.getElementById('gain-val').textContent = '0';
    applyCameraSetting();
}

// Stop preview when leaving camera tab
document.addEventListener('DOMContentLoaded', () => {
    const observer = new MutationObserver(() => {
        const camSection = document.getElementById('tab-camera');
        if (camSection && !camSection.classList.contains('active')) {
            stopCamPreview();
        }
    });
    const tabContainer = document.querySelector('.settings-sections') || document.querySelector('main');
    if (tabContainer) observer.observe(tabContainer, { subtree: true, attributes: true, attributeFilter: ['class'] });
});

// Start preview when camera tab is shown
const origTabHandler = window.handleTabClick;
const origNavTabs = document.querySelectorAll('.nav-tab-sm');
origNavTabs.forEach(tab => {
    tab.addEventListener('click', () => {
        if (tab.dataset.tab === 'camera') {
            setTimeout(() => { refreshCameraSettings(); startCamPreview(); }, 100);
        } else {
            stopCamPreview();
        }
    });
